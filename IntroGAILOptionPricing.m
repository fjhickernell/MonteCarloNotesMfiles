...
aPaths = genPaths(ourBrownianMotion,n) %an n by 250 matrix of numbers
...
ourOptionPayoff = optPayoff(inp) %construct an optPayoff object

%% 
% Note that the default is a European call option.  We generate some paths
% with the |genOptPayoffs| method:

methods(ourOptionPayoff)
n = 10; %number of paths to generate
Payoffs = genOptPayoffs(ourOptionPayoff,n) %an n vector of numbers
...
plot(ourOptionPayoff,n) %plot n paths
...
ourOptionPrice = optPrice(inp) %construct an optPrice object 
...
tic
Price = genOptPrice(ourOptionPrice) %the option price
toc
...
ourOptionPrice.timeDim.timeVector = 1
tic
Price = genOptPrice(ourOptionPrice) %the option price
toc

%%
% We can also change to a put option

ourOptionPrice.payoffParam.putCallType = {'put'}
tic
Price = genOptPrice(ourOptionPrice) %the option price
toc
