...
stockPaths(:,1)=param.assetParam.initPrice ...
   ...
   + bmPaths(:,1).*vol*sqrt(bmObj.timeDim.timeVector(1)));
for j=2:d
   vol=param.assetParam.volatility ...
   ...
   stockPaths(:,j)=stockPaths(:,j-1) ...
      .*exp((param.assetParam.interest-vol.*vol/2)*bmObj.timeDim.timeIncrement(j-1) + ...
      bmPaths(:,j).*vol*sqrt(bmObj.timeDim.timeIncrement(j-1)));
end
